        ...
        table1 = $('#table1').DataTable( {
            ...
            "columns": [
                ...
                { "data": "pagados" },
                {
                    ...
                    "render": function(data, type, row) {
                        ...
                        if (data == 0) {
                            return '<div class="progress progress-md mb-3"><div class="progress-bar bg-danger">' + data + '%</div></div>';
                        } else {
                            return '<div class="progress progress-md mb-3"><div class="progress-bar progress-bar-striped progress-bar-animated ' + colorClass + '" style="width: ' + data + '%">' + data + '%</div></div>';
                        }
                    }
                },
                { "data": "status" },
            ...
            "columnDefs": [
                ...
                },
                {
                    "targets": 11,
                    "render": function( data, type, row) {
                        var h = Mustache.render($("#tplDtColumnPagos").html(),
                                {
                                    row_id:  data.id,
            ...
            "createdRow": function (row, data, index) {
                ...
                if (status === '<?= RECORD_STATUS_ACTIVE_TEXT ?>') {
                    $(row).find('td:eq(10)').html('<span class="mb-0 text-success fs-13 font-weight-semibold">ACTIVO</span>');
                } else if (status === '<?= RECORD_STATUS_INACTIVE_TEXT ?>') {
                    $(row).find('td:eq(10)').html('<span class="mb-0 text-danger fs-13 font-weight-semibold">INACTIVO</span>');
                }
            },        
            language: {
