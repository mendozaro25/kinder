...
class Docentes extends MY_Controller {
	private $data;
	public function __construct()
	{
		parent::__construct();
		$this->data["page_title"] = "Docentes";
	}
	...
	public function docente() {
        $this->data["uri"] = [
            "title"         => "Docentes",
            "list"          => base_url().$this->name()."/docente_list",
            "create"        => base_url().$this->name()."/docente_create",
            "save"          => base_url().$this->name()."/docente_save",
            "remove"        => base_url().$this->name()."/docente_delete",
        ];
		$this->load->view($this->name()."/list", $this->data);
	}

    public function docente_list() {
		...
		$sql = "select 
					a.id, 
					concat(a.nombres,' ',a.apellidos) as docente,
					a.dni, 
					a.fecha_nac,
					a.email,
    ...
    public function docente_create() {
		...
		if ($id = $this->input->post("id"))
			$this->data = array_merge($data, gw("docente", ["id" => $id ])->row_array());
		
		$this->load->view($this->name() . "/create", $this->data);
	}

    public function docente_save() {
		$ret["success"] = false;
		...
		try {
			...
			if ($id > 0) {
				...
				$this->Constant_model->updateData("docente", $data, $id);

            } else {

				$obra_id = $this->Constant_model->insertDataReturnLastId("docente", $data);
    ...
    public function docente_delete() {
		$ret["success"] = false;
		$id = $this->input->post("id");
		try {
			if (!($record  = $this->Constant_model->getSingleOneColumn("docente", "id", $id)))
				throw new Exception("Registro no encontrado");
			$this->Constant_model->updateData("docente", ["status" => 0, "deleted_datetime" => date('Y-m-d H:i:s', time()) ], $id);
			$ret["success"] = true;
		} catch (Exception $ex) {
			$ret["message"] = $ex->getMessage();
		}
